...
// Load enrollment and append to request
...
const enrollmentById = async (req, res, next, id) => {
    try {
        ...
        return res.status('400').json({
            error: "Could not retrive enrollment"
        })
    }
}
...
const listEnrolled = async (req, res) => {
    try {
        let enrollments = await Enrollment.find({student: req.auth._id}).sort({'completed': 1})
...
export default { create, read, remove, complete, enrollmentById, isStudent, listEnrolled, findEnrollment, enrollmentStats }